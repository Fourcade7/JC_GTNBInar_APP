...
fun adminbottomNavGraphSetup(
    ...
    NavHost(navController = navHostController, startDestination = Screens.HomeAdmin.route ){
        composable(route = Screens.HomeAdmin.route){ homeAdminScreen(homeAdminViewModel,lazyPagingItems) }
        composable(route = Screens.PlansAdmin.route){ plansAdminScreen(plansAdminViewModel,lazyPagingItems) }
        composable(route = Screens.AcceptAdmin.route){ acceptAdminScreen(orderAdminViewModel) }
    }
}
